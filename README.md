...
Copy the **App ID** and the **PEM contents** →
`github_app_id` / `github_app_private_key`.
*Role: your own GitHub account (installation comes later and needs org owner).*

### Step 6 — Fill in the GitHub values
Paste the four `github_*` values from Steps 4–5 into `terraform.tfvars`
(the webhook secret is already in Terraform state — no need to set it).
